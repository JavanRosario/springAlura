...
public interface SwaggerDocControllers {
	...
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Filtro processado com sucesso!", content = @Content(mediaType = "application/json", examples = @ExampleObject(name = "Sucesso!", summary = "Exemplo quando o GET filtrado retorna dados", value = OpenApiExamples.SUCESSO_GET_JSON))),
			@ApiResponse(responseCode = "500", description = "Erro interno no servidor ao processar os filtros", content = @Content(mediaType = "application/json", examples = @ExampleObject(name = "Erro Interno", summary = "Exemplo de erro inesperado no sistema", value = OpenApiExamples.ERRO_GET_JSON))) })
	List<SerieResponseDto> listarComFiltros(
			@Parameter(description = "Objeto contendo os filtros dinâmicos de busca (Título e Preço Máximo)") com.springAlura.springAlura.domain.model.SerieFiltroDto filtros);
	...
	@ApiResponses({
			@ApiResponse(responseCode = "200", description = "Série updated com sucesso!", content = @Content(mediaType = "application/json", examples = @ExampleObject(name = "Sucesso!", summary = "Exemplo de quando o PUT é feito com sucesso", value = OpenApiExamples.SUCESSO_JSON))),
